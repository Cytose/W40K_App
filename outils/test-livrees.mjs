    ...
    return {
      ...
      simuChampsInconnus: [...new Set(STRAT_SIMU.flatMap(x =>
        Object.keys(x.effet || {}).filter(k => !(k in SIM.DEFAUT_STRAT))
          .map(k => x.nom + ' → ' + k)))],
      /* « faire quelque chose » ne veut pas dire « poser un chiffre » :
         un stratagème peut déclarer une situation — le Renforcement
         Synergétique prête le mot-clé CRYPTEK pour la phase, et ce sont
         les règles applicables qui changent derrière. Il porte alors
         `situ` au lieu d'`effet`. Ce qui reste interdit, c'est le
         stratagème qui ne fait ni l'un ni l'autre. */
      simuSansEffet: STRAT_SIMU.filter(x =>
        !x.situ && (!x.effet || !Object.keys(x.effet).length)).map(x => x.nom),
